...
if (isset($_REQUEST['ts'])) {
  // Need EC
  $tsParts = explode("|", mysqli_real_escape_string($link,hextostr($_REQUEST['ts'])));
  $sdate  = $tsParts[0];
  $edate  = $tsParts[1];
  $stime  = $tsParts[2];
  $etime  = $tsParts[3];
...
function level0() {
  ...
  $query = "SELECT COUNT(event.signature) AS f1,
    ...
    ORDER BY f5 $sv";
  /*
   * query for example in Workbench
   * 'SELECT COUNT(event.signature) AS f1,
    event.signature AS f2,
    event.signature_id AS f3,
    event.signature_gen AS f4,
    MAX(event.timestamp) AS f5,
    COUNT(DISTINCT(event.src_ip)) AS f6,
    COUNT(DISTINCT(event.dst_ip)) AS f7,
    event.ip_proto AS f8,
    GROUP_CONCAT(DISTINCT(event.status)) AS f9,
    GROUP_CONCAT(DISTINCT(event.sid)) AS f10,
    GROUP_CONCAT(event.status) AS f11,
    GROUP_CONCAT(event.timestamp),12,2 AS f12,
    event.priority AS f13,
    GROUP_CONCAT(DISTINCT(src_tag.value)) AS f14,
    GROUP_CONCAT(DISTINCT(dst_tag.value)) AS f15
    FROM event
...
function level1() {
  ...
  global $offset, $when, $sensors, $rt,$link;
  $sid = mysqli_real_escape_string($link, $_REQUEST['object']);
  $sv = mysqli_real_escape_string($link, $_REQUEST['sv']);
  $filter = hextostr($_REQUEST['filter']);
  ...
  if ($filter != 'empty') {
    if (substr($filter, 0,4) == 'cmt ') {
      ...
      $qp2 = "LEFT JOIN history ON event.sid = history.sid AND event.cid = history.cid 
        WHERE history.comment = '" . mysqli_real_escape_string($link, $comment[1]) . "'
  ...
  /*
   * example
   * 'SELECT COUNT(event.signature) AS count,
    MAX(event.timestamp) AS maxTime,
    INET_NTOA(event.src_ip) AS src_ip,
    msrc.c_long AS src_cc,
    INET_NTOA(event.dst_ip) AS dst_ip,
    mdst.c_long AS dst_cc,
    ...
    GROUP_CONCAT(event.cid) AS c_cid,
    GROUP_CONCAT(event.status) AS c_status,
    GROUP_CONCAT(event.timestamp,'+00:00'),12,5 AS c_ts,
    GROUP_CONCAT(event.timestamp, '+00:00'),12,2 AS f12,
    event.priority AS f13,
    msrc.age AS src_age,
    mdst.age AS dst_age,
    GROUP_CONCAT(DISTINCT(src_tag.value)) AS f14,
...
function level2() {
  ...
  global $offset, $when, $sensors, $rt,$link;
  $comp = mysqli_real_escape_string($link, $_REQUEST['object']);
  $filter = hextostr($_REQUEST['filter']);
  $sv = mysqli_real_escape_string($link, $_REQUEST['sv']);
  $adqp = mysqli_real_escape_string($link, hextostr($_REQUEST['adqp']));
  ...
  /*
   * example
   * 'SELECT event.status AS f1,
    CONCAT_WS(',',event.timestamp,'+00:00',event.timestamp) AS f2,
    INET_NTOA(event.src_ip) AS f3,
    event.src_port AS f4,
    INET_NTOA(event.dst_ip) AS f5,
    event.dst_port AS f6,
    ...
    WHERE history.comment = 'Hello world'
     AND (event.signature_id = 1000002
        AND event.src_ip = 3244117322
        AND event.dst_ip = 2887720711)
    GROUP BY event.sid,event.cid
...
function transcript() {
  ...
  $sqlsid = mysqli_real_escape_string($link, $sid);
  // Lookup sensorname
  ...
  $sensorName = mysqli_result($qResult, 0, $field =0);
...
function filters() {
  ...
  switch ($mode) {
    ...
    case "remove" :
      $alias =  mysqli_real_escape_string($link,$_REQUEST['data']);
...
function map() {
  ...
  $srcr = mysqli_query($link, $srcq);
  $dstr = mysqli_query($link, $dstq);
  ...
  while ($row = mysqli_fetch_row($srcr)) {
    $a1[] = $row[0];
    $a2[] = $row[1];
    $c1[] = $row[0];
    $c2[] = $row[1];
  ...
  while ($row = mysqli_fetch_row($dstr)) {
    $b1[] = $row[0];
    $b2[] = $row[1];
    if ($aHit == 'yes') {
...
function addremoveobject() {
    global $link;
  $user   = $_SESSION['sUser'];
  $obtype = mysqli_real_escape_string($link, $_REQUEST['obtype']);
  $object = mysqli_real_escape_string($link, hextostr($_REQUEST['object']));
...
function objhistory () {
  ...
  $result = mysqli_unbuffered_query($query);
...
function times() {
  ...
  global $offset, $when, $sensors, $link;
  $filter = hextostr($_REQUEST['filter']);
  if ($filter != 'empty') {
    $filter = str_replace('&lt;','<', $filter);
    $filter = str_replace('&gt;','>', $filter);
