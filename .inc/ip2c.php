...
function IP2C($string,$isCLI) {
    ...
    function lookup($list) {
        ...
        while ($row = mysqli_fetch_row($list)) {
            ...
            $ipLookup = mysqli_query("SELECT registry, cc, c_long, type, date, status FROM ip2c WHERE
            ...
            if ($result) {
                ...
                mysqli_query("REPLACE INTO mappings (registry,cc,c_long,type,ip,date,status)
    ...
    $db = mysqli_connect($dbHost,$dbUser,$dbPass) or die(mysqli_error());
    mysqli_select_db($dbName,$db) or die(mysqli_error());

    // Start timing
    $st = microtime(true);
    $sipList = mysqli_query("SELECT DISTINCT(e.src_ip) FROM event AS e LEFT JOIN mappings AS m ON e.src_ip=m.ip
                            WHERE (m.ip IS NULL OR m.cc = '01')");
    $dipList = mysqli_query("SELECT DISTINCT(e.dst_ip) FROM event AS e LEFT JOIN mappings AS m ON e.dst_ip=m.ip
                            WHERE (m.ip IS NULL OR m.cc = '01')");
    $sipCount = $dipCount = 0;
    if ($sipList) {
    ...
    $allRecs = mysqli_query("SELECT COUNT(*) FROM mappings");
